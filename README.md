...
ショートカット（アプリ・フォルダ・URL）、画像、動画、メモ、JSON などを自由に配置し、プロジェクト単位で一括管理できます。
...
- ネットワークドライブが固まるからアンマウントしておきたい人
- 作業プロジェクトごとにツールや資料のショートカット配置を切り替えたい人に
- デスクトップ切り替えアプリを使うほどじゃないんだよなっていう人に（常駐しない）
- Pythonでランチャーアプリを作って自由にカスタマイズでしたい人に（たたき台として）
- 動画を並べて閲覧したい人に（チュートリアルやお楽しみ動画を並べて使おう。再生位置をそれぞれ保存できます）
- ポータブルな作業環境（USB 等）を持ち運びたい人に
---
...
## コード・仕様につい (ご参考用)
- これは要件定義し、クラスやインターフェイスを設計し、それをAIに丸投げし動くように修正するの繰り返しで完成したものです。
- クラス派生・継承でoverrideしたいわけでもないのに重複してるメソッドがあるかもしれませんが、それはAIが書いたものだからです。意図したものではありません。
- 重複した関数やメソッドなど、気が付いたものは消したり統合したりしてますが、残っているかもしれません。適宜消してください。
  (でも私が書くと構造化されすぎて深淵のスパゲティになります。それに比べたらだいぶいいです。)
...
## 構想
- web-api 機能 (バッチファイル等のスクリプトに curl でコマンド仕込んで登録すれば現状でも可能）
- ネットワーク関連
  スクリプトにネットワークドライブのマウント・アンマウントを入れて登録しておくと便利です。
